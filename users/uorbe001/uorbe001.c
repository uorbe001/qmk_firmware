...
void space_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (xtap_state.state) {
    ...
    case DOUBLE_SINGLE_TAP: register_code(KC_SPACE); unregister_code(KC_SPACE); register_code(KC_SPACE);
...
void brackets_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (xtap_state.state) {
    ...
    case DOUBLE_SINGLE_TAP: register_code(KC_LBRACKET); unregister_code(KC_LBRACKET); register_code(KC_LBRACKET);
    //Last case is for fast typing. Assuming your key is `f`:
    //For example, when typing the word `buffer`, and you want to make sure that you send `ff` and not `Esc`.
    //In order to type `ff` when typing fast, the next character will have to be hit within the `TAPPING_TERM`, which by default is 200ms.
  }
...
void brackets_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (xtap_state.state) {
    ...
    case DOUBLE_SINGLE_TAP: unregister_code(KC_LBRACKET);
  }
  xtap_state.state = 0;
}
...
void super_grave_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (xtap_state.state) {
    ...
    case DOUBLE_TAP:
       ...
       break;
    case DOUBLE_SINGLE_TAP: register_code(KC_GRAVE); unregister_code(KC_GRAVE); register_code(KC_GRAVE);
    //Last case is for fast typing. Assuming your key is `f`:
    //For example, when typing the word `buffer`, and you want to make sure that you send `ff` and not `Esc`.
    //In order to type `ff` when typing fast, the next character will have to be hit within the `TAPPING_TERM`, which by default is 200ms.
  }
